<div class="track-container track-front shadow
    ...
    @if($disabled ?? false) track-past-week @endif"
     ...
     data-bs-toggle="tooltip" data-bs-html="true" title="{{ $schedule->trackTooltipText() }}"
    ...
    <div class="track-week track-ownership no-select {{ (!$schedule->track->free) ? 'track-ownership-red' : 'track-ownership-green' }}" data-track-id="{{ $schedule->track->package_id }}">{{ $schedule->race_week_num + 1 }}</div>
